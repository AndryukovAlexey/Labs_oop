...
import { evalCubicBezier, flattenCubicBezier, distanceToLineSegment } from "./curveUtils";
...
export class CubicBezier extends Shape {
  ...
  getLocalBounds(): Bounds {
    ...
    return boundsFromPoints([this.p0, this.p1, this.p2, this.p3, ...points]);
  }

  clone(): CubicBezier {
    const copy = new CubicBezier(0, 0, 0, 0, 0, 0, 0, 0);
  ...
  evalLocal(t: number): Point2D {
    return evalCubicBezier(t, this.p0, this.p1, this.p2, this.p3);
  }
  ...
  private getDevicePoints(): Point2D[] {
    ...
    // Преобразуем все локальные точки в экранные
