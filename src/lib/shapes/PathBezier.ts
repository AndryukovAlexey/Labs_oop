...
export class PathBezier extends Shape {
  ...
  hitTest(px: number, py: number): boolean {
    ...
    if (this.closed && devicePoints.length > 2) {
      ...
      // Замкнутый путь обычно залит, поэтому позволяем выбирать его за внутреннюю область
      if (this.pointInPolygon(px, py, devicePoints)) {
        return true;
      }
    }
  ...
  private pointInPolygon(px: number, py: number, polygon: Point2D[]): boolean {
    let inside = false;
